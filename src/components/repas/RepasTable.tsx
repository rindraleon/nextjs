...
interface RepasTableProps {
  data: Repas[];
  onEdit: (repas: Repas) => void;
  onDelete: (repas: Repas) => void;
}

export const RepasTable = ({ data, onEdit, onDelete }: RepasTableProps) => {
  return (
        ...
        <div className="min-w-[900px]">
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {data.map((repas) => (
                <TableRow key={repas.id}>
                  <TableCell className="px-2 py-1 sm:px-6 text-start">
                    <span className="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                      {repas.isManual ? "Manuel" : "Spécialisé"}
                    </span>
                  </TableCell>
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    {repas.isManual ? (
                      <div>
                        <div className="font-semibold">Repas Manuel</div>
                        <div>Présences: {repas.totalPresence || "N/A"}</div>
                      </div>
                    ) : (
                      repas.student && (
                        <div>
                          <div className="font-semibold">{repas.student.firstName} {repas.student.lastName}</div>
                          <div>Régime: {repas.student.dietaryRegime}</div>
                          <div>Classe: {repas.student.class?.name || "N/A"}</div>
                        </div>
                      )
                    )}
                  </TableCell>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                    <ul className="list-disc pl-5">
                      {repas.ingredients.map((ing, index) => (
                        <li key={index}>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                    <div className="flex gap-2">
                      <button onClick={() => onEdit(repas)} className="text-blue-600 hover:text-blue-800">
                        <EditIcon />
                      </button>
                      <button onClick={() => onDelete(repas)} className="text-red-600 hover:text-red-800">
                        <DeleteIcon />
                      </button>
                    </div>
                  </TableCell>
